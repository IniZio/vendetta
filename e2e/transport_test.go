...
`,
	...
	env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "create", "local-test")

	// Verify worktree was created
	worktreePath := filepath.Join(projectDir, ".vendatta", "worktrees", "local-test")
	_, err := os.Stat(worktreePath)
	require.NoError(t, err, "Worktree directory should exist")

	// Cleanup
	env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "rm", "local-test")
}
...
`,
		...
		output, err := env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "remote-test")
...
`,
		...
		_, err := env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "port-test")
		...
		// Cleanup
		env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "rm", "port-test")
	})
}
...
`,
	...
	worktreeConfig := filepath.Join(worktreePath, ".vendatta", "config.yaml")
...
`,
	...
	for i := 1; i <= 3; i++ {
		...
		env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "create", wsName)

		// Verify each workspace
		worktreePath := filepath.Join(projectDir, ".vendatta", "worktrees", wsName)
		_, err := os.Stat(worktreePath)
	...
	for i := 1; i <= 3; i++ {
		wsName := "multi-test-" + string(rune('a'+i-1))
		env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "rm", wsName)
	}
}
...
`,
		...
		_, _ = env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "empty-test")
...
`,
		...
		_, _ = env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "invalid-qemu")
		// Should not panic
	})
}
...
`,
		...
		output, err := env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "default-key-test")
...
`,
		...
		_, _ = env.RunVendattaCommandWithError(binaryPath, projectDir, "workspace", "create", "custom-key-test")
		// Fails at connection (expected), not config

		// Cleanup
		env.RunVendattaCommand(t, binaryPath, projectDir, "workspace", "rm", "custom-key-test")
	})
}
