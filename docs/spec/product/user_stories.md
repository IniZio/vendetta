...
- **Acceptance Criteria**:
    - Running `vendetta dev branch-name` creates a dedicated worktree.
    - Dependencies are installed inside a container, not on the host.
    - My existing `docker-compose` setup for the database works inside the vendetta environment (DinD).
...
> *As a frontend developer, I want to automatically know the URL of my backend API running in the vendetta environment so that I don't have to manually update my `.env.local` every time.*

- **Acceptance Criteria**:
    - vendetta discovers the host-mapped port for the `api` service.
    - The environment variable `vendetta_SERVICE_API_URL` is automatically injected into my frontend container.
...
- **Acceptance Criteria**:
    - Shared templates in `.vendetta/templates/` follow open standards
...
- **Acceptance Criteria**:
    - `vendetta kill session-id` destroys the container and the worktree.
